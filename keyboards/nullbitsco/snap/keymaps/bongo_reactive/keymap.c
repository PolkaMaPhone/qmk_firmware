...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    KC_MUTE, KC_GRV,   KC_1,    KC_2,   KC_3,   KC_4,   KC_5,   KC_6,     KC_7,   KC_8,   KC_9,    KC_0,      KC_MINS,   KC_EQL,   KC_BSPC,  KC_DEL,   KC_F13,
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {[_BASE] = {ENCODER_CCW_CW(KC_VOLD, KC_VOLU), ENCODER_CCW_CW(KC_F19, KC_F18)}, [_VIA1] = {ENCODER_CCW_CW(KC_BRID, KC_BRIU), ENCODER_CCW_CW(KC_MPRV, KC_MNXT)}, [_VIA2] = {ENCODER_CCW_CW(KC_NO, KC_NO), ENCODER_CCW_CW(KC_WBAK, KC_WFWD)}};
#endif
    ...
    switch (keycode) {
        case KC_F13:
            if (record->event.pressed) {
                // when keycode KC_F13 is pressed
                rgblight_toggle_noeeprom();
            } else {
                // when keycode KC_F13 is released
            }
        case KC_F18:
            if (record->event.pressed) {
                // when keycode KC_F18 is pressed
                rgblight_step_noeeprom();
            } else {
                // when keycode KC_F18 is released
            }
            break;
        case KC_F19:
            if (record->event.pressed) {
                // when keycode KC_F19 is pressed
                rgblight_step_reverse_noeeprom();
            } else {
                // when keycode KC_F19 is released
            }
            break;
        default:
            break;
